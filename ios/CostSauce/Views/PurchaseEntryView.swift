...
struct PurchaseEntryView: View {
    ...
    /// create-new state (Task 7); this is the one thing this screen needs
    /// back from it. `onClear` (fix round 1) is what makes this mirror the
    /// picker's OWN nil-ness again, so the "Purchase" section below hides
    /// the instant the chip is cleared, exactly as it did before extraction.
    ...
    var body: some View {
        Form {
            ...
            IngredientPickerView(
                appModel: appModel, excludedIngredientIds: [], onPick: pick,
                onClear: { selectedIngredient = nil },
                onQueryEdited: { savedMessage = nil }
            )
            .id(pickerResetToken)
    ...
    /// `.onChange(of: effectiveSelectionId)` handler's non-nil branch
    /// (same default-unit / `qtyInCase`-reset side effect). `savedMessage`
    /// is deliberately NOT touched here -- pre-extraction it was cleared
    /// only by `nameBinding`'s setter (every keystroke), never by a pick
    /// itself, so that responsibility now lives in `onQueryEdited` below,
    /// not here, to keep the restored timing exact.
